...
class Healthbar:
    ...
    def __init__(self, size: tuple[int, int], direction: str, offsetFromPos: tuple[int, int] = (0, 0),
                 fullColor: tuple[int, int, int] = (0, 255, 0), emptyColor: tuple[int, int, int] = (255, 0, 0)) -> None:
        self.size = size
    ...
    def Display(self, screen: pygame.Surface, pos: tuple[int, int]):
        ...
        emptyHealthbarRect = pygame.Rect(
            pos[0] + self.offsetFromPos[0] - self.size[0] / 2, pos[1] + self.offsetFromPos[1] - self.size[1] / 2,
            self.size[0], self.size[1])
        pygame.draw.rect(screen, self.emptyColor, emptyHealthbarRect)

        fullHealthbarInfo = self.GetFullHealthbar()
        fullHealthbarRect = pygame.Rect(
            math.ceil(pos[0] + self.offsetFromPos[0] + fullHealthbarInfo[0] - self.size[0] / 2),
            math.ceil(
